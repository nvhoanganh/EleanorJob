...
namespace Parser462
{
    ...
    {
        private static CrmConnector _connect {get;set;}
        
        public static void Process(string formResp, string mapperDef)
        {
            _connect = new CrmConnector();
    ...
    {
        string CreateEntity(string entityType, Dictionary<string, object> properties);
        string UpdateEntity(string id, string entityType, Dictionary<string, object> properties);
    ...
    {
        private readonly CrmConnector _connect;
        ...
        public string CreateEntity(string entityType, Dictionary<string, dynamic> properties)
        {
            ...
            // call crm service and save
            var recordId = _connect.ServiceClient.Create(newEntity);
        ...
        public string SearchCrmObject(string entityType, Dictionary<string, object> filters)
        {
            ...
            var query = new QueryByAttribute(entityType);
            ...
            var results = _connect.ServiceClient.RetrieveMultiple(query);
            return results.Entities?.ToList().FirstOrDefault()?.Id.ToString();
        }
        ...
        public string UpdateEntity(string id, string entityType, Dictionary<string, dynamic> properties)
        {
            ...
            // return the same id
            var newEntity = new Entity(entityType, Guid.Parse(id));
            ...
            _connect.ServiceClient.Update(newEntity);
            return id;
        }
    }
    #endregion
    ...
    class UpdateCrmEntity : IStep
    {
        private readonly string destinationEntity;
        private readonly string destinationId;
        private readonly ICrmService crmService;

        public UpdateCrmEntity(string destinationEntity, string id)
        {
            this.destinationEntity = destinationEntity;
        ...
        public Dictionary<string, dynamic> Process(Dictionary<string, dynamic> payload, Dictionary<string, dynamic> input, Dictionary<string, object> vars)
        {
            var actualId = HelperClass.Evaluate(destinationId, input, payload, vars);
            Console.WriteLine($"Id is: {actualId} {actualId.GetType()}");
            return new Dictionary<string, dynamic> {
                {"Crm_Id", this.crmService.UpdateEntity(actualId.ToString(), destinationEntity,  payload) }
            };
        }
    }
    ...
    static class HelperClass
    {
        ...
        public static object CreateComplexFieldType(JsonElement fieldDef, Dictionary<string, object> input, Dictionary<string, object> payload, Dictionary<string, object> vars)
        {
            if (fieldDef.Get("type").Value.ToString() == "CRM_ENTITY_REF")
            {
                ...
                {
                    LogicalName = fieldDef.Get("refType").Value.ToString(),
                    // value prop contains the expression which we need to evaluate
                    Id = Guid.Parse(HelperClass.Evaluate(fieldDef.Get("value").Value.ToString(), input, payload, vars))
                };
            }
        ...
        public static IStep StepFactoryCreate(JsonElement step)
        {
            ...
            if (step.Get("type").ToString() == "UPDATE_CRM_ENTITY") return new UpdateCrmEntity(step.Get("destEntityName").ToString(), step.Get("id").ToString());
